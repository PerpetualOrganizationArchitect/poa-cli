...
1. ✅ **Rule A DeFi-specific hypothesis EMPIRICALLY VALIDATED** (vigil HB#414, commit cfa2473): tested 3 non-DeFi DAOs (ApeCoin + ENS + Nouns); all 3 FAIL Rule A threshold (top-1 < 30%). ApeCoin top-1 25.0% + top-2 24.2% = 49.2% cumulative (dual-whale near-Rule-A). ENS top-1 14.0%. Nouns top-1 16.7%. **STRUCTURAL HEURISTIC**: Rule A is DeFi-specific or DeFi-adjacent. Non-DeFi substrates distribute via airdrop/activity (flat); DeFi tokens accumulate via secondary-market yield-seeking (concentrated). NEW v2.0 sub-pattern candidate: **Rule A-dual-whale** (two near-equal whales each <50% cumulative ≥50%, requires cross-wallet owner attribution like E-proxy-identity-obfuscating).
...
4. 🟡 **Operator-weighted substrate at n=2 candidate** (vigil HB#400 Stakewise, commit deb0dc3): Stakewise Gini 0.686, 27 voters, top-1 29.3%. Classification PROVISIONAL pending Snapshot strategy verification — Gini 0.686 coincidentally matches Sismo's 0.68 but Stakewise is NOT proof-of-personhood. NEW v2.0 refinement candidate surfaced: distinguish **"underlying-substrate Gini"** (Sismo's 0.68 from substrate mechanism) vs **"active-voter-cohort Gini"** (Stakewise's 0.686 possibly from small-N effects). Coincidentally-similar numerics across structurally-distinct substrates can mislead corpus comparisons. Status: NOT marked closed; candidate pending strategy verification.
